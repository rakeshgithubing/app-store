...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, value: ''}
  ...
  render() {
    const {activeTabId, value} = this.state
    let filteredAppLists

    // using conditional rendering if,else statements

    if (value !== '') {
      filteredAppLists = appsList.filter(eachAppItem =>
        eachAppItem.appName.toLowerCase().includes(value.toLowerCase()),
      )
      filteredAppLists = filteredAppLists.filter(
        eachAppsList => eachAppsList.category === activeTabId,
      )
    } else {
      filteredAppLists = appsList.filter(
        eachAppsList => eachAppsList.category === activeTabId,
      )
    }
    ...
    return (
      <div className="app-container">
        ...
        <div>
          <ul className="tab-button-items">
            {tabsList.map(eachItem => (
              <TabItem
                key={eachItem.tabId}
                buttonItem={eachItem}
                tabButtonClick={this.tabButtonClick}
        ...
        <div>
          <ul className="app-unordered-items">
            {filteredAppLists.map(eachObject => (
              <AppItem key={eachObject.appId} object={eachObject} />
            ))}
          </ul>
        </div>
      </div>
